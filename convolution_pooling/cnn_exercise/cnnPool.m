...
for imageNum = 1:numImages
    for featureNum = 1:numFeatures
        ...
        % data = convolvedFeatures(featureNum, imageNum, :, :);
        % rowMcol = (floor(convolveDim/poolDim)^2);
        % for i = 1:rowMcol
        %     offset = (i-1)*poolDim^2;
        %     window = data(:);
        %     pooledFeatures(featureNum, imageNum, ..., ...) = mean(window(offset+1:offset+poolDim^2));
        % end
    end
end
end
